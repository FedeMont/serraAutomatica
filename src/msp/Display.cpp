...
void Display::connecting()
{
    if (this->connectingFlag)
    {
        ...
        this->write(this->halfScreenX - textSize/2, this->halfScreenY - this->myScreen.fontSizeY(), text, orangeColour);
        this->myScreen.setOrientation(1);
        this->drawImage(connection, this->halfScreenY + this->myScreen.fontSizeY(), this->halfScreenX - connection.width/2);
        this->myScreen.setOrientation(0);
...
void Display::homeScreen(bool connectionState, String time, bool isMinutePassed, DayCycle dayCycle, float humidity, float temperature, bool shouldWatering, bool shouldFan, bool isLightAuto, bool isWaterAuto, bool isFanAuto)
{
    if (connectionState != this->previousConnectionState || isMinutePassed)
    {
        ...
        this->myScreen.setOrientation(1);
        this->drawImage(image, this->myScreen.fontSizeY(), this->screenSizeX - this->myScreen.fontSizeX() - image.width);
        this->myScreen.setOrientation(0);
    }
