...
WebServer::WebServer(FileSystem *fs) : server(80)
{
    this->fileSystem = fs;
}

WebServer::WebServer(FileSystem *fs, int port): server(port)
{
    this->fileSystem = fs;
}
...
void WebServer::save_json()
{
    ...
    if (!LittleFS.begin()) { // controllo di aver accesso al filesystem
        ...
        Serial.println(F("LittleFs Mount succesfull"));
    }
    ...
    {
#ifdef DEBUG
        Serial.println(F("Errore nella lettura della configurazione."));
#endif
    }
...
#ifdef DEBUG
    Serial.println(F("Riavvia il dispositivo."));
#endif
}